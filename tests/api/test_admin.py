...
def test_plan_crud(admin):
    body = {"name": "Team", "tagline": "pytest", "color": "blue", "days": 60, "aiDailyLimit": 20,
            "features": {"ai": True, "mcp": False, "sync": True, "secrets": True, "offline": False, "telemetry": True, "mail": True, "cloud": True}}
...
def test_feature_toggle(admin, user):
    uid = user.get(f"{BASE}/ai-api/auth/session", timeout=15).json()["user"]["id"]
    feats = {"ai": False, "mcp": True, "sync": True, "secrets": True, "offline": True, "telemetry": True, "mail": True, "cloud": True}
