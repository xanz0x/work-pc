...
// Usage: node desktop/scripts/smoke-packaged.mjs [port]
// Port: default from runtime.env; ПЕРЕДАВАЙТЕ другой, если живое приложение
// уже занимает дефолтный порт — иначе проверка попадёт в работающий сервер.
...
const PORT = String(process.argv[2] || env.WSX_NEXT_PORT)
...
const child = spawn(process.execPath, [path.join(serverDir, 'server.js')], {
  cwd: serverDir,
  env: { ...env, APP_PASSWORD: 'smoke-test-password-123', APP_SESSION_SECRET: 'ab'.repeat(24), MAIL_SECRET: 'cd'.repeat(24), ADMIN_LOGIN: 'admin', AI_DIR: path.join(sandbox, 'data'), NODE_ENV: 'production', HOSTNAME: '127.0.0.1', PORT, WSX_NEXT_PORT: PORT, SystemRoot: process.env.SystemRoot, TEMP: process.env.TEMP, TMP: process.env.TMP },
