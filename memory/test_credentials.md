...
- Из-за прод-сборки cookie входа помечена `Secure`, и `pytest tests/api` по
  ...
  https-адресу preview: `APP_URL=https://email-switcher-2.preview.emergentagent.com
...
- Тесты: `APP_URL=https://email-switcher-2.preview.emergentagent.com python3 -m pytest tests/api/test_mcp.py -q`;
...
- Тесты: `APP_URL=https://email-switcher-2.preview.emergentagent.com python3 -m pytest tests/api/test_sync.py -q`.
...
- Тесты: `npx vitest run tests/unit/users.test.ts`; `APP_URL=https://email-switcher-2.preview.emergentagent.com python3 -m pytest tests/api/test_admin.py tests/api/test_plans_licensing.py -q`; e2e `tests/e2e/23-admin-accounts.spec.ts`. Подробно — `/app/auth_testing.md`.
...
- Прогон: `APP_URL=https://email-switcher-2.preview.emergentagent.com python3 -m pytest tests/api/test_mail_read.py -q` (тест лимита — последний, съедает бюджет минуты).
