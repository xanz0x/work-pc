...
import '@/app/styles/chat-refinement.css'
...
import { actionResult, checkChatAction, executeChatAction } from '@/lib/chat-actions'
import { chatPasswordRefs, clearChatPasswords, readChatPassword } from '@/lib/chat-passwords'
...
export function ScreenChat() {
  ...
  const exec = useCallback(
    async (name: string, args: Record<string, unknown>, session: string, signal: AbortSignal): Promise<ExecResult> => {
      // Recheck after approval: an administrator may have revoked access while waiting.
      const denied = await checkChatAction(name, args, signal)
      if (signal.aborted) return actionResult(false, 'Действие остановлено.')
      if (denied) return actionResult(false, denied)
      const result = await executeChatAction(name, args, session, D, signal)
      if (result) return result
      if (name === 'find_file') {
        if (!v.settings.toggles.sendIndex) return actionResult(false, 'Передача метаданных файлов отключена в настройках приватности.')
        ...
        const scored = v.views
          ...
          .filter((x) => !toks.length || x.score > 0)
      ...
      if (name === 'save_password') {
        ...
        const password = args.password_ref ? readChatPassword(String(args.password_ref), session) : String(args.password ?? '')
        if (!title || !url) return actionResult(false, 'Укажите название и адрес сайта для сохранения.')
        if (!password || password === '•••') return actionResult(false, 'Пароль недоступен. Сгенерируйте его снова в этом диалоге; запись не создана.')
        try { const parsed = new URL(url); if (!['https:', 'http:'].includes(parsed.protocol)) throw new Error() } catch { return actionResult(false, 'Укажите корректный адрес сайта с https:// или http://.') }
        ...
        if (!secrets.ready) return actionResult(false, 'Сейф закрыт — разблокируйте мастер-ключ.')
        const fields = [
          ...
          { name: 'Пароль', kind: 'password' as const, value: password, secret: true },
        ...
        const key = `skill:save_password:${session}:${title}:${url}:${String(args.login ?? '')}`
        const run = await M.runExclusive(
          key,
          async () => { const problem = await secrets.createEntry('login', title, fields, { tags: ['ИИ'] }); if (problem) throw new Error(problem) },
          { dedupMs: 5 * 60_000 },
        )
        if (!run.ok) {
          const already = run.reason === 'duplicate'
          return {
            ok: already,
        ...
        return {
          ok: true,
          summary: `Запись «${title}» сохранена; пароль зашифрован`,
    ...
    [M, v, secrets, D, redactIds],
  )

  const ai = useAiChat(exec)
  useEffect(() => () => clearChatPasswords(), [])
  useEffect(() => { if (lock.status === 'locked') { clearChatPasswords(); ai.stop() } }, [lock.status, ai.stop])

  /* ---------- запуск хода ---------- */

  const buildCtx = useCallback(
    (pinned: string[], session: string): TurnBody['ctx'] => ({
      passwordRefs: chatPasswordRefs(session),
      /* P0-1: индекс сейфа уходит наружу только с разрешения пользователя. */
      files: v.settings.toggles.sendIndex
        ? v.views.filter((f) => !redactIds.has(f.id)).map((f) => ({ id: f.id, name: f.name, cat: f.cat, tags: fileTags(f) }))
        : [],
      pinned: v.settings.toggles.sendIndex ? pinned.filter((id) => !redactIds.has(id)).map((id) => v.viewById(id)?.name ?? id) : [],
      lock: secrets.needsLock ? 'не настроен' : 'настроен (содержимое секретов ИИ недоступно)',
      scanned: v.stats.files,
    }),
    [v, secrets.needsLock, redactIds],
  )
  ...
  const run = useCallback(
    (
      ...
      const ctx = buildCtx(pinned, sessionId)
      ...
      ai.start(
        ...
        if (view.isCloud && !r.errorCode && !r.stopped) {
          v.notify({
            ...
            }. Результаты разрешённых навыков также переданы модели. Сгенерированные локально пароли передаются только ссылкой.`,
    ...
    (text: string) => {
      const fire = () => {
        ...
        if (!active) {
          ...
          runLatest.current(s.id, { text, title: s.title })
          return
        }
        patch(active.id, (s) => ({
          ...s,
        ...
        runLatest.current(active.id, { text, title: active.msgs.length === 0 ? text.slice(0, 42) : active.title })
      }
  ...
  return (
    <div data-testid="screen-chat" className={`chat chat-refined${railOpen ? ' has-rail' : ''}${deskMsg ? ' has-desk' : ''}`}>
      {railOpen ? (
      ...
      <section className="chat-main" role="main" aria-label="Разговор с архивом">
        <header className="chat-top">
          {!railOpen ? (
            <button
              ...
              data-testid="chat-show-sessions"
            >
          ...
          <div className="chat-titles">
            <h1 className="chat-title" data-testid="chat-title">{active?.title ?? 'Новый диалог'}</h1>
            <p className="chat-sub mono ellipsis" data-testid="chat-engine-sub">
              {engine.engineView.model} · {engine.engineView.label}
          ...
          <button
            ...
            title={
              engine.engineView.isCloud
                ...
                : 'Локальная модель; почтовые навыки обращаются к своему провайдеру'
            }
        ...
        {finding ? (
          <div className="chat-find">
            ...
            />
            <span className="mono chat-find-count" data-testid="chat-find-count">
        ...
        >
          <div className="chat-thread" aria-busy={streaming}>
            {msgs.length === 0 && !live ? (
              <div className="chat-empty">
                ...
                <h2 className="chat-empty-title" data-testid="chat-empty-title">Что нужно сделать?</h2>
                <p className="chat-empty-note" data-testid="chat-empty-description">
                  {engine.engineView.isCloud
                    ? 'Работайте с файлами, почтой и секретами через чат. Изменения выполняются после подтверждения, с учётом ваших прав.'
                    : engine.engineView.ready
                      ? 'Модель отвечает локально. Почтовые навыки обращаются к провайдеру; изменения требуют подтверждения.'
                      : 'Выбран локальный движок, но он не отвечает. Ниже — что запустить; или переключите режим в настройках.'}
                </p>
                <ul className="chat-sugs">
                  {['Какие действия мне доступны?', 'Сгенерируй почту Gmail', 'Сгенерируй пароль из 20 символов', 'Найди файл в библиотеке'].map((s, i) => (
                    <li key={s}>
                      <button type="button" className="f-chip" data-testid={`chat-suggestion-${i}`} onClick={() => setDraft(s)}>
                        {s}
                      </button>
                    </li>
                  ))}
                </ul>
                <p className="chat-empty-stat" data-testid="chat-privacy-note">
                  Не вставляйте существующие пароли в сообщения. Генератор показывает новый пароль только в браузере.
                </p>
              </div>
            ) : null}
            ...
            {msgs.length && !finding ? (
              <div className="chat-day">
                <span className="label-mono" data-testid="chat-history-label">Переписка</span>
              </div>
            ) : null}
            ...
            {finding && needle.trim() && shown.length === 0 ? (
              <p className="chat-nofind" data-testid="chat-find-empty">В этом разговоре нет такого текста.</p>
            ) : null}
          ...
          {fresh && !atBottom ? (
            <button type="button" className="jump" data-testid="chat-jump-bottom" onClick={jumpDown}>
