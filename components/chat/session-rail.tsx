...
import { IconPlus, IconTrash, IconChat, IconChevronLeft, IconSearch, IconPencil } from '../icons'
import './chat-rail.css'
  ...
  return (
    <div className="rail">
      <header className="rail-head">
        <span className="label-mono" data-testid="chat-sessions-heading">Диалоги</span>
        <span className="grow" />
        <button type="button" className="icon-btn" onClick={onNew} aria-label="Новый диалог" data-testid="chat-rail-new">
        ...
        <button
          ...
          data-testid="chat-hide-sessions"
        >
      ...
      {found.length === 0 ? (
        <p className="rail-empty" data-testid="chat-sessions-empty">
          {sessions.length === 0
              ...
              <li key={s.id} className={s.id === activeId ? 'is-on' : undefined}>
                {editId === s.id ? (
                  ...
                  <button
                    ...
                    title={`${s.title} · двойной клик — переименовать`}
                ...
                <button
                  type="button"
                  className="rail-del rail-ren"
                  onClick={() => {
                    setEditId(s.id)
                    setDraft(s.title)
                  }}
                  data-testid={`chat-session-rename-btn-${s.id}`}
                  aria-label={`Переименовать диалог «${s.title}»`}
                  title="Переименовать"
                >
                ...
                <button
                  type="button"
                  className="rail-del"
                  onClick={() => { if (window.confirm(`Удалить диалог «${s.title}»? Историю нельзя будет восстановить.`)) onDelete(s.id) }}
      ...
      <p className="rail-foot" data-testid="chat-history-storage-note">История хранится в браузере и в вашей учётной записи на сервере.</p>
    </div>
  )
}
