      ...
      <form className="mk-card is-danger" {...dialogProps} onPointerDown={(e) => e.stopPropagation()}
        ...
        <div className="mk-body">
          ...
          <div className="access-field">
            ...
            <textarea id="reset-lock-confirmation" className="access-input access-confirmation" rows={1} value={confirmation} onChange={(e) => setConfirmation(e.target.value)}
              onKeyDown={(e) => {
                // Unlike input, textarea preserves pasted newlines for exact matching.
                if (e.key === 'Enter' && !e.shiftKey && !e.nativeEvent.isComposing) {
                  e.preventDefault()
                  if (confirmation === 'СБРОСИТЬ') e.currentTarget.form?.requestSubmit()
                }
              }}
              aria-describedby={/[\r\n]/.test(confirmation) ? 'reset-lock-paste-error' : undefined}
              autoComplete="off" spellCheck={false} data-testid="reset-lock-confirmation" />
            {/[\r\n]/.test(confirmation) && <p id="reset-lock-paste-error" className="access-error" role="alert" data-testid="reset-lock-paste-error">Введите СБРОСИТЬ без пробелов и переносов строки.</p>}
          </div>
        </div>
