...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  return (
    <>
          ...
          <div className="sidebar-foot">
            ...
            <div className="sidebar-storage" data-testid="sidebar-storage">
              <div className="storage-head">
                ...
                <span className="num label-mono" data-testid="storage-size">
                  {fmtBytes(stats.bytes)}
                </span>
              </div>
              <div className={`storage-meta num${stats.processing > 0 ? ' busy' : ''}`} role="status" aria-label={stats.processing > 0 ? 'Идёт обработка файлов' : undefined} data-testid="storage-usage">
                {stats.files} {plural(stats.files, 'файл', 'файла', 'файлов')}
                {stats.processing > 0 ? ` · обрабатывается: ${stats.processing}` : ''}
              </div>
            </div>
      ...
      {v.palette && v.lock.status !== 'locked' && <CommandPalette />}
