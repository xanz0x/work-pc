...
import '@/app/styles/map-refinement.css'
...
export function ScreenMap() {
  ...
  useEffect(() => {
    ...
    function safe() {
      // Desktop HUD измеряется в тех же координатах, что и canvas, включая масштаб UI.
      if (matchMedia('(hover: hover) and (pointer: fine)').matches) {
        const stage = cv!.getBoundingClientRect()
        const root = cv!.parentElement!
        const inspector = root.querySelector('.node-inspector')?.getBoundingClientRect()
        const controls = root.querySelector('.map-topleft')?.getBoundingClientRect()
        const footer = root.querySelector('.map-footer')?.getBoundingClientRect()
        const gap = 16 * (stage.width / Math.max(1, cv!.clientWidth))
        if (inspector && controls && footer) return {
          l: gap,
          r: stage.right - inspector.left + gap,
          t: controls.bottom - stage.top + gap,
          b: stage.bottom - footer.top + gap,
        }
      }
      const narrow = W <= 470
      const wide = W > 900
    ...
    function select(n: Node | null, found = false) {
      ...
      if (n.core) {
        setInfo({
          ...
          meta: 'Центральный узел карты связей.',
          links: spokes.length,
          power: Math.round(60000 / (search && search.phase !== 'settled' ? BEAT_FAST_MS : BEAT_MS)),
          powerLabel: 'Ритм анимации',
          hue: WAVE,
          clusterId: null,
          cluster: 'Сводка графа',
          typeLabel: 'ЯДРО',
          core: true,
        })
        return
    ...
    function build() {
      ...
      for (const gn of g.nodes) {
        ...
        nodes.push({
          ...
          shared: sharedIdsRef.current.has(gn.id),
  ...
  return (
    <div className="map-stage map-space" role="main" data-testid="screen-map">
      ...
      <canvas id="net" ref={canvasRef} aria-label="Карта связей файлов и стикеров" data-testid="map-canvas" />

      <div className="float-panel map-topleft" data-testid="map-toolbar">
        <div className="map-controls glass" role="group" aria-label="Управление картой" data-testid="map-controls">
        ...
        >
          ...
          <span className="flow-text mono" title={flowText} data-testid="map-flow-text">
            {flowText}
          </span>
      ...
      <div className="float-panel map-footer glass" data-testid="map-footer">
        <div className="map-legend" aria-label="Легенда карты" data-testid="map-legend">
          <span className="lg" data-testid="map-legend-links">
            <i style={{ background: 'rgba(47,190,126,.95)' }} />
            ядро · {stats.links} связей
          </span>
          <span className="lg" data-testid="map-legend-files">
            <i style={{ background: 'rgba(107,118,137,.9)' }} />
            файлы · {stats.files}
          </span>
          <span className="lg" data-testid="map-legend-notes">
      ...
      >
        <div className="ni-head" ref={headRef} tabIndex={-1} data-testid="map-inspector-heading">
          <span className="label-mono">Инспектор узла</span>
          <button
            className="icon-btn ni-close"
            title="Закрыть (Esc)"
            aria-label="Закрыть инспектор"
            data-testid="map-inspector-close"
            onClick={() => call('clear')}
          >
            <IconClose />
          </button>
        </div>
        <div className="ni-scroll" data-testid="map-inspector-content">
        {info ? (
          <>
            <div className="ni-file">
              ...
              <div className="ni-file-text">
                <div className="ni-title-row">
                  ...
                  <span className="ni-badge mono" data-testid="map-node-type">{info.typeLabel}</span>
                </div>
                <p data-testid="map-node-meta">{info.meta}</p>
              </div>
            </div>
            <span className="ni-cluster" data-testid="map-node-cluster">
              <i className="net-dot" style={{ background: `rgb(${info.hue})`, animation: 'none' }} />
              {info.cluster}
              {info.found && <b className="ni-found mono" data-testid="map-node-found">цель волны</b>}
            </span>

            <div className="ni-metrics">
              <div className="ni-metric">
                <span className="ni-m-label" data-testid="map-node-links-label">{info.core ? 'Магистралей' : 'Связей'}</span>
                <span className="ni-m-val mono num" data-testid="map-node-links">{info.links}</span>
              </div>
              <div className="ni-metric">
                <span className="ni-m-label" data-testid="map-node-power-label">{info.powerLabel}</span>
                <span className="ni-m-val mono num" data-testid="map-node-power">
            ...
            {!info.core && neighbors.length > 0 && (
              <div className="ni-section">
                <span className="label-mono" data-testid="map-neighbors-heading">
                ...
                <div className="ni-links">
                  {neighbors.map((nb) => (
                    ...
                    >
                      ...
                      <span className="ni-link-name">{nb.node.label}</span>
            ...
            {info.core && topClusters.length > 0 && (
              <div className="ni-section">
                <span className="label-mono" data-testid="map-core-clusters-heading">Кластеры</span>
                <div className="ni-links">
                  {topClusters.map((c) => (
                    <button
                      className="ni-link"
            ...
            <details className="ni-section ni-activity" data-testid="map-activity">
              <summary className="label-mono" data-testid="map-activity-toggle">
                Анимация связей <IconChevronDown aria-hidden="true" />
              </summary>
              <div className="wave-log wave-tl mono" data-testid="map-wave-log">
                {rows.map((r) => {
                  const idx = PHASE_ORDER.indexOf(r.phase)
                  const state = pi > idx ? ' done' : pi === idx ? ' on' : ''
                  return (
                    <div className={`step${state}${r.accent ? ' accent' : ''}`} key={r.t} data-testid={`map-wave-step-${r.phase}`}>
              ...
              {history.length > 0 && (
                <div className="ni-trace">
                  <span className="label-mono" data-testid="map-trace-heading">Узлы этого сеанса</span>
                  {history.map((h) => (
                    <button
                    ...
                    >
                      <i aria-hidden="true" />
                      <span className="ni-trace-name">{h.name}</span>
          ...
          <div className="ni-empty" data-testid="map-empty-state">
            <p className="ni-empty-title" data-testid="map-empty-title">Узел не выбран</p>
            <p data-testid="map-empty-stats">Узлов: {stats.nodes} · связей: {stats.links}<br />Объём: {fmtBytes(stats.bytes)}</p>
        ...
        {/* Действия вне прокрутки: доступны даже при длинном имени и увеличенном масштабе. */}
        {info && (
          <div className="ni-actions" data-testid="map-node-actions">
            {!info.core && (
              <button
            ...
            >
              <IconLayers /> {info.core ? 'Все файлы' : 'Кластер'}
            </button>
          </div>
        )}
      </aside>
