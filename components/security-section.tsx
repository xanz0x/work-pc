...
import { IconAlertTri, IconCheck, IconClose, IconKey, IconLockRound } from './icons'
...
export function SecuritySection() {
  ...
  /* Одноразовый код восстановления, выданный при последнем setup/change.
     Пока он на экране — форма ключа закрыта карточкой выдачи кода. */
  ...
  async function onCreate(e: React.FormEvent) {
    ...
    /* Успех → одноразовый код восстановления прямо в этом окне;
       неудача → null, подробности уходят уведомлением в ленту сейфа. */
    const code = await v.setupLock(s1, method)
    if (code === null) {
      setErr('Не удалось создать мастер-ключ — подробности в ленте сейфа')
      return
    }
    setIssuedCode(code)
  }
  ...
  async function onChangeMaster(e: React.FormEvent) {
    ...
    /* Успех → НОВЫЙ код восстановления (старый недействителен). */
    const code = await v.changeMaster(curSecret, next1)
    if (code === null) {
      setErr('Не удалось изменить мастер-ключ — подробности в ленте сейфа')
      return
    }
    setIssuedCode(code)
  }
  ...
  const createModal = creating ? (
    <MkModal
      ...
      sub={issuedCode ? 'Замок включён — сохрани ключ восстановления' : 'Для блокировки сейфа и защиты секретов'}
      onClose={resetForms}
      onSubmit={onCreate}
      testId="mk-modal"
      footer={
        issuedCode ? (
          <button
            ...
            data-testid="mk-recovery-done"
          >
            <IconCheck width={13} height={13} aria-hidden="true" focusable="false" />
            Я записал код
          </button>
        ) : (
        <>
    ...
    >
      {issuedCode ? (
        <RecoveryCodeCard code={issuedCode} testId="mk-recovery-code" />
      ) : (
      <>
  ...
  const changeModal = changing ? (
    <MkModal
      ...
      sub={issuedCode ? 'Новый ключ сохранён — код восстановления' : isPin ? 'Текущий и новый PIN' : 'Текущий и новый пароль'}
      onClose={resetForms}
  ...
  if (lock.status === 'off') {
    return (
      <section className="sec panel" id="set-security" data-testid="settings-security-section">
        <div className="sec-head">
          <span className="sec-icon">
            <IconLockRound />
          </span>
          <div className="sec-head-text">
            <h2 className="setting-title" data-testid="settings-security-title">Безопасность</h2>
  ...
  return (
    <section className="sec panel" id="set-security" data-testid="settings-security-section">
      ...
      {changeModal}
      {disableModal}
      {/* Замок уже мог включиться (status off → unlocked): модалка создания
          с карточкой кода восстановления должна дожить до «Я записал код». */}
      {createModal}
    </section>
  )
}
