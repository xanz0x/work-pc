...
export function ScreenVault() {
  ...
  if (!s.ready) {
    /* P8: замок уже открыт, а сессия секретов ещё не поднята — показываем
       честный статус и кнопку перезапуска вместо вечного «закрыт». */
    const waiting = v.lock.status === 'unlocked'
    return (
      <div className="vt vt-gate" data-testid="vault-gate-wait">
        <div className="panel vt-gate-card">
          ...
          <p className="vt-note" data-testid="vault-wait-description">
            {waiting
              ? 'Замок открыт — дожидаемся ключа сейфа секретов. Обычно это секунды.'
          ...
          {waiting && (
            <button
              ...
              data-testid="vault-wait-retry"
            >
              Проверить ключ снова
            </button>
          )}
        </div>
      </div>
