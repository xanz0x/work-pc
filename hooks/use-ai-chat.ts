...
export type ExecResult = {
  passwordRef?: string
  link?: { section: string; setting?: string }
  ok: boolean
  content: string
  summary: string
...
export function useAiChat(
  exec: (name: string, args: Record<string, unknown>, session: string, signal: AbortSignal) => Promise<ExecResult>,
) {
  ...
  useEffect(() => () => { decision.current?.(false); abortRef.current?.abort() }, [])
  ...
  const loop = useCallback(
    async (body: Record<string, unknown>, signal: AbortSignal): Promise<void> => {
      ...
      if (st.current.tools.length >= 20) throw new TurnError('BAD_REQUEST')
      let res: Response
      try {
      ...
      for (const c of calls) {
        const run: ToolRun = {
          id: c.id,
          name: c.name,
          label: CHAT_TOOLS[c.name]?.label ?? c.name,
          args: c.name === 'save_password' ? { ...c.args, password: '•••' } : { ...c.args },
          status: 'run',
        }
        s.tools = [...s.tools, run]
        syncTools()
        ...
        const denied = await checkChatAction(c.name, c.args, signal)
        if (signal.aborted) return
        if (denied) {
          run.status = 'err'
          run.summary = denied
          syncTools()
          results.push({ id: c.id, name: c.name, content: JSON.stringify({ ok: false, error: denied }) })
          continue
        }
        if (CHAT_TOOLS[c.name]?.confirm) {
          run.status = 'wait'
          syncTools()
          setPending(run)
          ...
          if (!ok) {
            ...
            results.push({
              id: c.id,
              name: c.name,
              content: JSON.stringify({ ok: false, error: 'Пользователь отклонил действие. Не повторяй его без новой просьбы.' }),
            })
            continue
          }
          run.status = 'run'
        ...
        let r: ExecResult
        try {
          r = await execRef.current(c.name, c.args, String(body.sessionId), signal)
        } catch (e) {
          r = {
            ok: false,
  ...
  const start = useCallback(
    (body: TurnBody, onDone: (r: TurnResult) => void) => {
      ...
      const finalize = (errorCode?: AiErrorCode) => {
        const s = st.current
        for (const t of s.tools) if (t.status === 'wait' || t.status === 'run') { t.status = 'deny'; t.summary = 'Действие остановлено; завершение не подтверждено' }
