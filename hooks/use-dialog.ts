...
export function useDialog<T extends HTMLElement = HTMLDivElement>({
  ...
  useLayoutEffect(() => {
    // Capture the opener before a parent's passive autofocus effect runs.
    if (open) returnToRef.current = document.activeElement instanceof HTMLElement ? document.activeElement : null
  }, [open])
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    function onFocus(e: FocusEvent) {
      if (!modal || stack[stack.length - 1] !== node || !node || node.contains(e.target as Node)) return
    ...
    return () => {
      ...
      requestAnimationFrame(() => {
        const top = stack[stack.length - 1]
        if (returnTo && document.contains(returnTo) && (!top || top.contains(returnTo))) {
