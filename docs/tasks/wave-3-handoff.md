# Волна 3 · Передача дел — **ВОЛНА ВЫПОЛНЕНА** 2026-06-02

> Файл-инструкция для продолжения в новом чате. Обновлено 06.2026 (волна закрыта).
> Кодовая часть волны 3 закрыта целиком, включая хвост из трёх доработок.
> Единственный открытый пункт — ручная приёмка NF-1 на папке ~1 000 файлов.
...
| **LG-3 · Журнал безопасности** | `DONE` 2026-06-02 | Append-only стор `journal` (схема БД v3), панель с фильтром и выгрузкой, уведомления ссылаются на запись. Юнит + e2e зелёные. |
...
Кодовая часть волны 3 закрыта. Осталась только ручная приёмка NF-1.

### 2.1 NF-1 — ручная приёмка (не код, а проверка)
...
### 2.2 Узкие подписки — закрыто 2026-06-02
Все пять экранов ушли с фасада: карта и библиотека (ранее), затем замок
(`useLockStore`), настройки (`useSettingsStore` + данные + уведомления + навигация
+ тост + движок) и чат (композиция `data` + `settings` + `nav` плюс `notify`,
`flash`). `useVault()` остался только у каркаса.
...
## 4. Ключевые точки LG-3 (журнал безопасности)

- Хранилище: стор `journal` в IndexedDB (схема v3, autoIncrement) — отдельно от
  документов, поэтому wipe сейфа, очистка уведомлений и retention его не касаются.
- API: `lib/journal.ts` — `logJournal(kind, title, detail) → id`, `readJournal()`,
  `subscribeJournal()`, `journalToFile()`. Удаления и очистки нет намеренно.
- Точки записи: `lib/store/lock.tsx` (включение/смена/выключение/сброс),
  `lib/store/data.tsx` (wipe), `lib/store/settings.tsx` (согласие на облако),
  `lib/secrets-store.tsx` (экспорт без шифрования, восстановление бэкапа),
  `components/screen-chat.tsx` (ИИ сохранил пароль), `hooks/use-ai-chat.ts`
  (каждый исходящий ход; локальные не логируются), `components/onboarding.tsx`
  (отказ от мастер-ключа).
- Ссылка из уведомления: `link: { kind: 'journal', id }` → `openNotif` ведёт в
  настройки и подсвечивает строку (`settingFocus = journal:<id>`).
- UI: `components/journal-panel.tsx`, раздел `set-journal`, стили в
  `app/styles/screen-settings.css` (слой `wf031`).
- `data-testid`: `journal-section`, `journal-count`, `journal-rows`, `journal-row`,
  `journal-empty`, `journal-filter-all`, `journal-filter-<kind>`, `journal-export`.

---

## 5. Ключевые точки NF-4 (онбординг)
...
## 6. Ключевые точки NF-2 (локальный движок)
...
- ~~E2E `03-chat-tool-call` нестабилен~~ — разобрано 2026-06-02: клик уходил в
  ещё не гидратированную кнопку навигации. Каркас теперь публикует
  `data-app-ready`, e2e ждут его через `waitAppReady()`, чанки грузятся с
  повторами. Все три сценария зелёные, полный набор 16/16.
- `/ai-api/chat` ограничен 10 ходами в минуту: чат-набор нельзя гонять с
  `--repeat-each` ≥ 5, иначе сценарии закономерно ловят `RATE_LIMITED`.
