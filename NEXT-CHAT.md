...
- **iteration_6 (100%, замечаний нет) — сессия MAP v2:** карта памяти переработана —
  кнопка «Проиграть поиск» убрана, волны активации идут сами (планировщик уступает человеку,
  ручной клик закрепляет карточку), фон-сетка заменена на космос (`.cosmos` + звёзды и метеоры
  на канвасе), инспектор получил реальные связи узла, магистрали ядра и журнал сеанса.
...
### P1 — карта памяти (после сессии MAP v2)
5. Разбить `components/screen-map.tsx` (~1.9 тыс. строк) на модули: планировщик автоволн,
   рендер канваса (звёзды/метеоры/граф), инспектор, обвязка UI. Долг из code-review iteration_6.
...
| `components/screen-map.tsx` | карта памяти: автопоток волн, космос, инспектор связей |
