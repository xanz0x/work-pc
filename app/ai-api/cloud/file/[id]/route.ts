...
export const DELETE = withRoute('/ai-api/cloud/file/[id]', async (req: NextRequest, ctx: Ctx) => {
  const { id } = await ctx.params
  const body = (await req.json().catch(() => ({}))) as { removeBytes?: unknown }
  try {
    await deleteFile(id, { removeBytes: body.removeBytes === true })
    return NextResponse.json({ ok: true })
  } catch (e) {
    return cloudError(e)
  }
