...
import { MASTER_CHANGED_EVENT } from './lock-store'
...
export type SecretsCtx = {
  ...
  hideAll: () => void
  /**
   * P8: ручная попытка поднять сессию ключа сейфа секретов — для gate-экрана,
   * когда замок открыт, а сессия ещё не подтвердилась.
   */
  retrySession: () => void
}
...
export function SecretsProvider({ children }: { children: ReactNode }) {
  ...
  /* ---------- P8: надёжный перезапуск сессии ключа сейфа секретов ----------
     После changeMaster/recoverWithKey обёртка SEK переупакована под нового
     мастера, и сессию надо поднять заново. Деривация PBKDF2 идёт заметно
     дольше кадра, поэтому окно попыток — ~3 секунды (25 × 120 мс), а не
     прежние 1.2 с, которые проигрывали гонку с adoptMasterSession. */
  const sessionEpochRef = useRef(0)

  /** Цикл попыток ensureSecretsSession; новая попытка отменяет предыдущую. */
  const runSessionAttempts = useCallback(async (): Promise<boolean> => {
  ...
  const journalSessionFailure = useCallback(() => {
    void logJournal(
      'secrets-session',
      'Сессия сейфа секретов не открыта',
      'Ключ сейфа секретов не восстановлен за ~3 секунды: сеанс мастера не принят или обёртка ключа не открылась. Модуль секретов закрыт до повторной разблокировки.',
    )
  }, [])
  ...
  useEffect(() => {
    if (status !== 'unlocked') {
      sessionEpochRef.current += 1 // гасим идущий цикл попыток
      dropSecretsSession()
      setReady(false)
      return
    }
    let alive = true
    void runSessionAttempts().then((ok) => {
      if (!alive) return
      if (ok) setReady(true)
      else {
        setReady(false)
        journalSessionFailure()
      }
    })
    return () => {
      alive = false
  ...
  }, [status, v.lockEpoch, runSessionAttempts, journalSessionFailure])

  /* P8: смена мастера (changeMaster/recoverWithKey) присылает локальное
     событие — пересобираем сессию секретов в той же вкладке, не дожидаясь
     смены статуса замка (его при смене мастера не происходит). */
  useEffect(() => {
    ...
    window.addEventListener(MASTER_CHANGED_EVENT, onMasterChanged)
    return () => window.removeEventListener(MASTER_CHANGED_EVENT, onMasterChanged)
  }, [runSessionAttempts, journalSessionFailure])

  /* P8: ручная попытка с gate-экрана — замок открыт, а сессия не подтвердилась. */
  const retrySession = useCallback(() => {
    if (statusRef.current !== 'unlocked') return
    void runSessionAttempts().then((ok) => {
      if (ok) setReady(true)
      else journalSessionFailure()
    })
  }, [runSessionAttempts, journalSessionFailure])
