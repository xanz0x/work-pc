...
export type VaultFile = {
   ...
   */
  cloudShared?: boolean
  /** Подпапка внутри локальной папки хранения ('' — её корень). */
  dir?: string
...
/* ------------------------------------------------------------
   ПОДКЛЮЧЁННАЯ МОДЕЛЬ
   Локального списка моделей больше нет: отвечает та модель,
   которую владелец подключил в настройках (OpenRouter или свой
   OpenAI-совместимый сервер). Здесь живёт только подпись,
   пока сервер не сообщил настоящее имя.
   ------------------------------------------------------------ */
