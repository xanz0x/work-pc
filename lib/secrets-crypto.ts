...
import { aesDecrypt, aesEncrypt, b64ToBytes, bytesToB64, randomBytesOf, readLockState } from './crypto-vault'
import { getMasterSession } from '@/hooks/use-file-keys'

export const SECRETS_SEK_KEY = 'wf.secrets.sek.v1'

type SekBlob = { v: 1; wct: string; wiv: string; salt?: string }
...
function writeSek(blob: SekBlob): void {
  try {
    /* Обёртка штампуется солью текущего lock-state: по штампу ниже узнаём,
       что SEK остался от стёртого мастера (замок удалили и создали заново),
       а не от здоровой смены мастера с переупаковкой (P8). */
    const salt = readLockState()?.saltB64
    localStorage.setItem(SECRETS_SEK_KEY, JSON.stringify(salt ? { ...blob, salt } : blob))
  } catch {
 ...
 * false = нет сеанса мастера или обёртка SEK принадлежит текущему мастеру,
 * но не открывается (честная крипто-неудача — ждём повторных попыток).
 */
export async function ensureSecretsSession(): Promise<boolean> {
  ...
  if (blob) {
    ...
    if (rawB64 === null) {
      /* Сеанс мастера есть, а обёртка SEK не открывается. Здоровая смена
         мастера переупаковывает обёртку (rewrapAll), поэтому так бывает,
         только когда SEK осиротел: замок удалили и создали заново (wipe
         стирает wf.lock.*, но не wf.secrets.sek.v1), rewrap не удался или
         обёртка повреждена. Старое сырьё в этих случаях невосстановимо —
         честно минтуим новое: модуль остаётся рабочим, записи старого SEK
         не раскрываются. Устаревший штамп соли это подтверждает. */
      const st = readLockState()
      const orphaned = !blob.salt || !st || blob.salt !== st.saltB64
      if (!orphaned) return false
    } else {
      /* Обёртка старого формата без salt-штампа — доштампуем при удачном чтении. */
      if (!blob.salt) writeSek({ v: 1, wct: blob.wct, wiv: blob.wiv })
