...
import type { CloudLibraryItem } from '../cloud-types'
...
export type VaultStats = {
  ...
  links: number
  nodes: number
  /** Суммарный объём файлов сейфа — показывается как есть, без квоты. */
  bytes: number
  processing: number
  sessions: number
  model: string
...
export function DataProvider({ children }: { children: ReactNode }) {
  ...
  const loadCloud = useCallback(async () => {
    ...
    try {
      ...
      setCloudNotes(d.files.filter((f) => f.kind === 'note' && f.note).map((f) => ({
        ...
        title: f.note!.title, body: f.note!.body, tags: f.note!.tags,
        createdAt: new Date(f.at).getTime(), expiresAt: null, lifeSpan: null, locked: false, secret: null,
      })))
      setCloudFiles(
        d.files.filter((f) => f.kind !== 'note').map((f) => {
  ...
  useEffect(() => {
    ...
    return () => { window.removeEventListener('wsx:cloud-changed', h); window.removeEventListener('focus', h) }
  }, [loadCloud])
