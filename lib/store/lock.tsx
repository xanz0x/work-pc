...
export type LockCtx = {
  ...
  /**
   * Включить замок. Успех → одноразовый код восстановления (строка),
   * неудача → null (подробности уходят в ленту сейфа уведомлением).
   */
  setupLock: (secret: string, method: LockMethod) => Promise<string | null>
  /**
   * Смена мастера с перезапаковкой. Успех → НОВЫЙ код восстановления
   * (старый перезаписан), неудача → null (подробности — уведомлением).
   */
  changeMaster: (
  ...
  /**
   * Вход по коду восстановления (замок заблокирован, мастер-ключ забыт):
   * код расшифровывает старый мастер-секрет, далее цепочка changeMaster
   * под новый секрет — данные остаются целыми. null = успех,
   * строка = причина отказа. Запись кода после успеха удаляется.
   */
  recoverWithKey: (
    recoveryCode: string,
    newSecret: string,
  ...
  /** Неудачные попытки ввода кода восстановления в этой вкладке. */
  recoveryFails: number
  /** Анти-брутфорс восстановления: ввод заблокирован до этого мгновения. */
...
export function LockProvider({ children }: { children: ReactNode }) {
  ...
  /* ---------- анти-брутфорс восстановления ---------- */

  /** Пять неверных кодов входят без кулдауна; дальше — как у мастера (failDelayMs). */
  const RECOVERY_MAX_FAILS = 5
  const [recoveryFails, setRecoveryFails] = useState(0)
  const [recoveryCooldownUntil, setRecoveryCooldownUntil] = useState(0)
  const recoveryGuardRef = useRef({ fails: 0, cooldownUntil: 0 })

  const noteRecoveryFail = useCallback(() => {
    const fails = recoveryGuardRef.current.fails + 1
    const cooldownUntil = fails > RECOVERY_MAX_FAILS ? Date.now() + failDelayMs(fails) : 0
    recoveryGuardRef.current = { fails, cooldownUntil }
    setRecoveryFails(fails)
    setRecoveryCooldownUntil(cooldownUntil)
    return cooldownUntil
  }, [])

  const clearRecoveryGuard = useCallback(() => {
    recoveryGuardRef.current = { fails: 0, cooldownUntil: 0 }
    setRecoveryFails(0)
  ...
  const setupLock = useCallback(
    async (secret: string, method: LockMethod): Promise<string | null> => {
      if (lockRef.current.busy) return null
      ...
      if (!cryptoAvailable()) {
        notify({
          ...
          icon: 'lockRound',
          title: 'Мастер-ключ не создан',
          body: 'WebCrypto недоступен в этом браузере — замок не работает',
        })
        return null
      }
      ...
      try {
        ...
        /* Ключ восстановления создаётся здесь же: код возвращается вызывающему
           и показывается ровно один раз. Хранится не мастер, а строка
           мастер-секрета под отдельным 256-битным recovery-ключом. */
        const recoveryCode = await createRecoveryForSecret(secret)
        ...
        notify({
          ...
          icon: 'lockRound',
          title: 'Замок включён',
          body: `Мастер-ключ (${method === 'pin' ? 'PIN' : 'пароль'}) создан на этом устройстве. PBKDF2 · 600 000 итераций. Выдан ключ восстановления — запишите его.`,
        })
        void logJournal(
          'lock-setup',
          'Замок включён',
          `Мастер-ключ создан на этом устройстве (${method === 'pin' ? 'PIN' : 'пароль'}), PBKDF2 · 600 000 итераций. Выдан одноразовый ключ восстановления.`,
        )
        return recoveryCode
      } catch (e) {
        setLock((p) => ({ ...p, busy: false }))
        const msg = e instanceof Error ? e.message : 'Не удалось создать мастер-ключ'
        notify({
          kind: 'danger',
          cat: 'privacy',
          icon: 'lockRound',
  ...
  const changeMaster = useCallback(
    async (currentSecret: string, nextSecret: string, nextMethod?: LockMethod) => {
      if (lockRef.current.busy) return null
      const cfg = readLockConfig()
      if (!cfg?.enabled || !cryptoAvailable()) {
        notify({
          kind: 'danger',
          cat: 'privacy',
          icon: 'lockRound',
          title: 'Мастер-ключ не изменён',
          body: 'Замок не настроен',
        })
        return null
      }
      setLock((p) => ({ ...p, busy: true }))
      try {
        ...
        if (oldKey && nextState) {
          try {
            ...
            await rewrapAll(oldKey, newKey, notesRef.current, patchNoteSecret, nextState.saltB64)
          } catch {
            /* переупаковка не удалась — сообщим ниже, данные не потеряны */
          }
        }
        await adoptMasterSession(nextSecret)
        /* Код восстановления перезаписывается под новый мастер с НОВЫМ
           recovery-секретом: старый код становится недействительным. */
        const recoveryCode = await createRecoveryForSecret(nextSecret)
        writeLockConfig({ ...cfg, enabled: true, method })
        activityRef.current = Date.now()
        clearRecoveryGuard()
        postLockSync('unlock-config-changed') // п.10.9: вкладки перечитают конфиг
        notifyMasterChanged() // P8: менеджер секретов в ЭТОЙ вкладке пересобирает сессию
        setLock((p) => ({
          ...p,
          status: 'unlocked',
          method,
        ...
        notify({
          ...
          body: 'Верификатор пересоздан с новой солью. Выдан новый ключ восстановления — запишите его.',
        })
        void logJournal(
          'master-changed',
          'Мастер-ключ изменён',
          `Верификатор пересоздан с новой солью, метод: ${method === 'pin' ? 'PIN' : 'пароль'}. Обёртки файловых ключей и секретов переупакованы, ключ восстановления перезаписан.`,
        )
        return recoveryCode
      } catch (e) {
        setLock((p) => ({ ...p, busy: false }))
        const msg = e instanceof Error ? e.message : 'Не удалось изменить мастер-ключ'
        notify({
          kind: 'danger',
          cat: 'privacy',
          icon: 'lockRound',
    ...
    [notify, notesRef, patchNoteSecret, clearRecoveryGuard],
  )

  /* ---------- вход по коду восстановления ---------- */

  /**
   * «Не помню мастер-ключ», но код на руках. Код расшифровывает старый
   * мастер-секрет → проверка верификатором → та же цепочка, что changeMaster:
   * setMasterSecret(новый) → rewrapAll(старый ключ → новый) → adoptMasterSession.
   * Защищённые файлы и секретные записки остаются целыми; запись кода
   * удаляется (одноразовый), новый код выдаётся при следующем changeMaster.
   */
  const recoverWithKey = useCallback(
    async (
      ...
      if (recoveryGuardRef.current.cooldownUntil > Date.now()) {
        return 'Слишком много неверных кодов — попытка заблокирована'
      }
      const method = nextMethod ?? cfg.method
      const policyError = validateSecret(newSecret, method)
      if (policyError) return policyError
      setLock((p) => ({ ...p, busy: true }))
      const failWrongCode = (): string => {
        const cooldownUntil = noteRecoveryFail()
        setLock((p) => ({ ...p, busy: false }))
        notify({
          kind: 'warn',
          cat: 'privacy',
          icon: 'lockRound',
          title: `Неудачная попытка восстановления (${recoveryGuardRef.current.fails})`,
          body:
            cooldownUntil > Date.now()
              ? `Код не подходит. Следующая попытка через ${Math.max(1, Math.ceil((cooldownUntil - Date.now()) / 1000))} с.`
              : 'Код не подходит. Проверка шла локально.',
        })
        return 'Код не подходит'
      }
      try {
        const oldSecret = await recoverMasterSecret(recoveryCode)
        if (!oldSecret) return failWrongCode()
        if (!(await verifyMasterSecret(oldSecret))) return failWrongCode()
        /* Ключ старого мастера: им расшифрованы обёртки до перезаписи состояния. */
        const prevState = readLockState()
        if (!prevState) return failWrongCode()
        const oldKey = await deriveMasterKey(
          oldSecret,
          b64ToBytes(prevState.saltB64),
          prevState.iterations,
        )
        await setMasterSecret(newSecret) // свежая соль + верификатор, счётчики в ноль
        resetFailures()
        /* Всё, что было завёрнуто прежним мастером (файловые ключи, секреты
           стикеров, ключ сейфа секретов), переупаковывается под новый —
           protected-данные остаются доступны с новым мастер-ключом. */
        const nextState = readLockState()
        if (nextState) {
          try {
            const newKey = await deriveMasterKey(
        ...
        await adoptMasterSession(newSecret)
        removeRecoveryBlob() // код одноразовый: использован и удалён
        writeLockConfig({ ...cfg, enabled: true, method })
        activityRef.current = Date.now()
        clearRecoveryGuard()
        postLockSync('unlock-config-changed') // п.10.9: вкладки перечитают конфиг
        ...
        notify({
          ...
          body: 'Мастер-ключ заменён, защищённые данные целы. Код восстановления использован — новый выдаётся при смене мастер-ключа.',
        })
        void logJournal(
          'lock-recovered',
          'Доступ восстановлен ключом',
          'Вход по одноразовому ключу восстановления: мастер-секрет извлечён из записи wf.lock.recovery, мастер заменён без потери данных. Код использован.',
        )
        return null
      } catch (e) {
        setLock((p) => ({ ...p, busy: false }))
        return e instanceof Error ? e.message : 'Не удалось восстановить доступ'
      }
    },
    [notify, notesRef, patchNoteSecret, noteRecoveryFail, clearRecoveryGuard],
  )
  ...
  const completeUnlock = useCallback(() => {
    ...
    notify({
      ...
      body: 'Мастер-ключ проверен. Сейф разблокирован.',
    })
  }, [notify])
  ...
  const resetLock = useCallback(() => {
    ...
    void logJournal(
      'lock-reset',
      'Замок сброшен',
      `Путь «забыл мастер-ключ»: верификатор и ${lostKeys} обёрток файловых ключей стёрты без проверки прежнего мастер-ключа. Файлы остались, пароли к ним — нет.`,
    ).then((jid) =>
      notify({
        kind: 'danger',
        cat: 'privacy',
  ...
  const value = useMemo<LockCtx>(
    ...
    [
      lock, lockEpoch, fileKeysCount, setupLock, changeMaster, recoverWithKey,
      recoveryFails, recoveryCooldownUntil, disableLock, lockNow, unlock,
