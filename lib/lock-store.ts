...
/** Есть ли у замка одноразовый ключ восстановления (запись wf.lock.recovery). */
export function hasRecoveryCode(): boolean {
  return rawGet(RECOVERY_KEY) !== null
}
 ...
 * и одноразовая запись ключа восстановления — код от стёртого замка мёртв.
 */
...
/* ---------- перезапуск сессии секретов в ЭТОЙ вкладке (P8) ----------
   postLockSync слышат только ЧУЖИЕ вкладки. После changeMaster/recoverWithKey
   менеджер секретов в той же вкладке обязан пересобрать свой сеанс: обёртка
   SEK переупакована под нового мастера, а его сессия могла остаться от
   прежнего ключа. Локальное window-событие без хранения состояния. */
export const MASTER_CHANGED_EVENT = 'wf.lock.master-changed'

export function notifyMasterChanged(): void {
  try {
    window.dispatchEvent(new Event(MASTER_CHANGED_EVENT))
  } catch {
    /* нет window (SSR-прогон) — слушателей просто нет */
  }
}
