/* ============================================================
   ...
   плана docs/architecture/lock-system.md), ссылка обнуляется после lockNow.
...
/* ============================================================
   КЛЮЧ ВОССТАНОВЛЕНИЯ (схема BitLocker) · wf.lock.recovery
   Под отдельным 256-битным recovery-секретом (код 'WSXR-…') лежит
   AES-GCM-шифртекст САМОГО мастер-секрета (строки пароля/PIN).
   Сам мастер нигде не хранится открыто, но строка секрета восстанавливаема:
   unlock проверяет её верификатором, а adoptMasterSession(secret) воссоздаёт
   ключ из секрета — поэтому для входа достаточно знать мастер-секрет из кода.
   Блоб одноразовый: после успешного восстановления удаляется, новый код
   выдаётся при следующем changeMaster. Мастер-пароль кодом НЕ является:
   код открывает запись, из которой строка секрета читается целиком.
   ============================================================ */

export const RECOVERY_KEY = 'wf.lock.recovery'
export const RECOVERY_CODE_PREFIX = 'WSXR-'
/** 32 случайных байта → 43 символа base64url. */
const RECOVERY_SECRET_BYTES = 32

export type RecoveryBlob = {
  v: 1
  /** Соль деривации ключа восстановления (отдельная от мастера). */
  saltB64: string
  ctB64: string
  ivB64: string
  iterations: number
...
/* base64url (без '=': 32 байта дают ровно 43 символа). */
...
function b64urlToBytes(s: string): Uint8Array | null {
  ...
  try {
    return fromB64(norm + '='.repeat((4 - (norm.length % 4)) % 4))
  } catch {
    return null
  }
}

function isRecoveryBlob(x: unknown): x is RecoveryBlob {
  if (typeof x !== 'object' || x === null) return false
  const b = x as Record<string, unknown>
  return (
    b.v === 1 && isB64(b.saltB64) && isB64(b.ctB64) && isB64(b.ivB64) &&
    typeof b.iterations === 'number' && b.iterations > 0
  )
}

export function readRecoveryBlob(): RecoveryBlob | null {
  try {
    const raw = store().getItem(RECOVERY_KEY)
    if (!raw) return null
    const parsed: unknown = JSON.parse(raw)
    return isRecoveryBlob(parsed) ? parsed : null
  } catch {
    return null
  }
}

function writeRecoveryBlob(blob: RecoveryBlob): void {
  try {
    store().setItem(RECOVERY_KEY, JSON.stringify(blob))
  } catch {
    /* приватный режим — код восстановления проживёт только в момент выдачи */
  }
}

export function removeRecoveryBlob(): void {
  try {
...
/** 'WSXR-' + 43 символа base64url — ровно такой код выдаётся пользователю. */
export function recoveryCodeFromBytes(bytes: Uint8Array): string {
  return RECOVERY_CODE_PREFIX + bytesToB64url(bytes)
}

/** null для любого ввода, кроме 'WSXR-' + 43 корректных символов base64url. */
export function recoveryBytesFromCode(code: string): Uint8Array | null {
  const raw = code.trim()
  if (!raw.startsWith(RECOVERY_CODE_PREFIX)) return null
  const body = raw.slice(RECOVERY_CODE_PREFIX.length)
  if (body.length !== 43) return null
  const bytes = b64urlToBytes(body)
  return bytes && bytes.length === RECOVERY_SECRET_BYTES ? bytes : null
}

/**
 * Создать запись восстановления под текущий мастер-секрет и вернуть код.
 * Вызывается при setupLock и changeMaster (код показывается один раз).
 * Бросает исключение при недоступности storage — вызывающий решает,
 * считать ли включение замка неудачей.
 */
export async function createRecoveryForSecret(
  masterSecret: string,
  iterations: number = LOCK_ITERATIONS,
): Promise<string> {
  const secret = randomBytes(RECOVERY_SECRET_BYTES)
  const salt = randomBytes(SALT_BYTES)
  const key = await deriveMasterKey(bytesToB64url(secret), salt, iterations)
  const { ctB64, ivB64 } = await aesEncrypt(key, JSON.stringify({ masterSecret }))
  writeRecoveryBlob({ v: 1, saltB64: toB64(salt), ctB64, ivB64, iterations })
  return recoveryCodeFromBytes(secret)
}

/**
 * Расшифровать запись кодом восстановления → старый мастер-секрет.
 * null при неверном коде, отсутствующей/повреждённой записи (GCM-тег).
 */
export async function recoverMasterSecret(code: string): Promise<string | null> {
  const blob = readRecoveryBlob()
  if (!blob) return null
  const secret = recoveryBytesFromCode(code)
  if (!secret) return null
  const key = await deriveMasterKey(
    bytesToB64url(secret),
    fromB64(blob.saltB64),
    blob.iterations,
  )
  const pt = await aesDecrypt(key, blob.ctB64, blob.ivB64)
  if (pt === null) return null
  try {
    const parsed = JSON.parse(pt) as { masterSecret?: unknown }
    return typeof parsed.masterSecret === 'string' ? parsed.masterSecret : null
  } catch {
    return null
  }
}
