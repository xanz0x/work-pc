...
export async function listSkills(): Promise<SkillFile[]> {
  ...
  for (const [tool, spec] of Object.entries(CHAT_TOOLS)) {
    if (all.some((s) => s.tool === tool)) continue
    all.push({ id: tool.replaceAll('_', '-'), name: spec.label, kind: 'tool', tool, builtin: true, enabled: tool !== 'notion_pull', description: spec.description, instructions: '' })
  }
