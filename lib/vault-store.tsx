...
export type VaultCtx = {
   ...
   * неудача → null (подробности — уведомлением в ленту сейфа).
   */
  changeMaster: (
  ...
  /** Вход по коду восстановления: null = успех, строка = причина отказа. */
  recoverWithKey: (
    recoveryCode: string,
    newSecret: string,
    nextMethod?: LockMethod,
...
function VaultFacade({ children }: { children: ReactNode }) {
  ...
  const value = useMemo<VaultCtx>(
    () => ({
      ...
      recoverWithKey: L.recoverWithKey,
      recoveryFails: L.recoveryFails,
      recoveryCooldownUntil: L.recoveryCooldownUntil,
