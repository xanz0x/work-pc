 ...
 * newSaltB64 — соль НОВОГО lock-state: ею штампуется обёртка SEK, чтобы
 * ensureSecretsSession мог отличить осиротевший SEK (замок пересоздавали)
 * от здоровой переупаковки (P8).
 */
  ...
  if (sekRaw) {
    try {
      ...
      if (typeof blob.wct === 'string' && typeof blob.wiv === 'string') {
        const next = await reWrap(oldKey, newKey, { ct: blob.wct, iv: blob.wiv })
        if (next) {
          const stamped = newSaltB64
            ? { ...blob, wct: next.ct, wiv: next.iv, salt: newSaltB64 }
            : { ...blob, wct: next.ct, wiv: next.iv }
          lsSet(SECRETS_SEK_KEY, JSON.stringify(stamped))
          report.sek = true
        } else {
          report.broken++
        }
...
export async function migrateKdfIterations(
  ...
  try {
    ...
    const report = await rewrapAll(oldKey, newKey, notes, patchNoteSecret, bytesToB64(salt))
